...
function scheduleSummary(status, config) {
    ...
    if (mode === "daylight") {
        if (status?.in_schedule === false)
            return "daylight only · waiting for sunrise";
    ...
    if (status?.in_schedule === true)
        return `active \u00b7 ${hours.length} hours/day`;
    return `${hours.length} hours/day`;
}

function deriveScheduleMode(config) {
    if (
        config?.schedule_mode === "daylight" ||
        config?.schedule_mode === "hours" ||
        config?.schedule_mode === "scene"
    )
        return config.schedule_mode;
...
function rssiQuality(dbm) {
    if (dbm == null || !Number.isFinite(dbm))
        return { bars: 0, label: "no signal", tone: "var(--soft)" };
    if (dbm >= -55)
        return { bars: 4, label: "excellent", tone: "var(--green)" };
...
async function renderCamera(root, hash, isActive = () => true) {
    const cameraId = decodeURIComponent(
        hash.replace(/^#\/cameras\/([^/]+).*/, "$1"),
    );
    ...
    const framesState = {
        ...
        groupBy:
            window.localStorage?.getItem(`frames:${cameraId}:groupBy`) ||
            "hour",
        density:
            window.localStorage?.getItem(`frames:${cameraId}:density`) || "",
        cursor: null,
        loading: false,
        error: "",
        warping: false,
    ...
    async function load() {
        try {
            ...
            if (!camera) {
                renderTopbar(
                    [{ label: "Dashboard", href: "#/dashboard" }, "Unknown"],
                    "",
                );
            ...
            const reinitTokenPending =
                dslrCfg0.reinit_token &&
                dslrCfg0.reinit_token !== dslrSt0.last_reinit_token;
    ...
    function paint() {
        ...
        renderTopbar(
            [{ label: "Dashboard", href: "#/dashboard" }, display],
            `
    ...
    `;
        ...
        root.querySelector('[data-action="render"]')?.addEventListener(
            "click",
            async () => {
        ...
        root.querySelector('[data-action="toggle-pause"]')?.addEventListener(
            "click",
            togglePause,
        );
          ...
          <div class="cam-canvas">
            ...
            ${
                status.last_capture_at
                    ? `<div class="cam-overlay-br"><span class="stamp">${escapeHtml(status.last_capture_at.replace("T", " · ").slice(0, 18))}</span></div>`
                    : ""
            }
          </div>
    ...
    function wireFrames() {
        ...
        root.querySelector("#frames-group-by")?.addEventListener(
            "click",
            (event) => {
                ...
                window.localStorage?.setItem(
                    `frames:${cameraId}:groupBy`,
                    framesState.groupBy,
                );
                paintFrames();
            },
        );
        root.querySelector("#frames-density")?.addEventListener(
            "click",
            (event) => {
                ...
                window.localStorage?.setItem(
                    `frames:${cameraId}:density`,
                    framesState.density,
                );
                paintFrames();
            },
        );
        root.querySelector("#frames-search")?.addEventListener(
            "input",
            paintFrameRail,
        );
        root.querySelector("#frames-refresh")?.addEventListener("click", () =>
            loadFrameArchive({ keepDay: true }),
        );
        root.querySelector("#frames-warp-toggle")?.addEventListener(
            "click",
            toggleFrameWarp,
        );
        root.querySelector("#frames-scrubber")?.addEventListener(
            "mousemove",
            updateFrameWarpPreview,
        );
        root.querySelector("#frames-scrubber")?.addEventListener(
            "mouseleave",
            () => {
        ...
        root.querySelector("#frames-grid-scroll")?.addEventListener(
            "scroll",
            () => {
                window.requestAnimationFrame(updateFrameScrubberWindow);
            },
        );
        root.querySelector("[data-clear-selection]")?.addEventListener(
            "click",
            clearFrameSelection,
        );
        root.querySelector("[data-delete-selection]")?.addEventListener(
            "click",
            deleteSelectedFrames,
        );
        root.querySelector("[data-render-selection]")?.addEventListener(
            "click",
            async () => {
                const { openRenderModal } =
                    await import("/static/v2/views/library.js");
    ...
    function removeFrameGlobalListeners() {
        if (frameMousemoveHandler)
            window.removeEventListener("mousemove", frameMousemoveHandler);
        if (frameMouseupHandler)
            window.removeEventListener("mouseup", frameMouseupHandler);
        if (frameKeydownHandler)
            window.removeEventListener("keydown", frameKeydownHandler);
        ...
        try {
            ...
            if (
                ...
                !framesState.days.some(
                    (day) => day.day === framesState.activeDay,
                )
            ) {
                framesState.activeDay = framesState.days[0]?.day || "";
            }
            if (framesState.activeDay)
                framesState.monthOpen.add(framesState.activeDay.slice(0, 7));
            paintFrameRail();
            if (framesState.activeDay)
                await loadFrameDay(framesState.activeDay);
            else {
    ...
    async function loadFrameDay(day) {
        ...
        framesState.daySummary =
            framesState.days.find((entry) => entry.day === day) || null;
        ...
        try {
            ...
            const data = await api.fetchJson(
                `/api/cameras/${encId}/frames?${params}`,
            );
    ...
    function paintFrameLoading(message) {
        ...
        if (grid)
            grid.innerHTML = `<div class="loading mono small">${escapeHtml(message)}</div>`;
    }

    function paintFrameRail() {
        const rail = root.querySelector("#frames-archive-rail");
        if (!rail) return;
        const query = (root.querySelector("#frames-search")?.value || "")
            .trim()
            .toLowerCase();
        ...
        rail.innerHTML =
            railHtml ||
            `<div class="empty"><h3>No matches</h3><p>No captured days match that search.</p></div>`;
        rail.querySelectorAll("[data-month]").forEach((button) => {
            button.addEventListener("click", () => {
                const month = button.dataset.month;
                if (framesState.monthOpen.has(month))
                    framesState.monthOpen.delete(month);
        ...
        rail.querySelectorAll("[data-frame-day]").forEach((button) => {
            button.addEventListener("click", () =>
                loadFrameDay(button.dataset.frameDay),
            );
        });
    }
    ...
    function paintFrames() {
        ...
        grid.innerHTML =
            groups.map(renderFrameGroup).join("") +
            `<div class="frames-grid-tail"></div>`;
        grid.querySelectorAll("[data-frame-cursor]").forEach((tile) => {
            tile.addEventListener("mousedown", (event) =>
                event.stopPropagation(),
            );
            tile.addEventListener("click", (event) =>
                selectFrameFromEvent(event, tile.dataset.frameCursor),
            );
            tile.addEventListener("dblclick", () =>
                openFrameLightbox(tile.dataset.frameCursor),
            );
        });
    ...
    function updateFrameToolbar() {
        ...
        if (title)
            title.textContent = framesState.activeDay
                ? formatDayTitle(framesState.activeDay)
                : "No frames";
        ...
        root.querySelectorAll("#frames-group-by button").forEach((button) =>
            button.classList.toggle(
                "active",
                button.dataset.group === framesState.groupBy,
            ),
        );
        root.querySelectorAll("#frames-density button").forEach((button) =>
            button.classList.toggle(
                "active",
                button.dataset.density === framesState.density,
            ),
        );
        root.querySelector("#frames-warp-toggle")?.classList.toggle(
            "primary",
            framesState.warping,
        );
    }
    ...
    function buildFrameGroups() {
        const gapsByAfter = new Map(
            (framesState.daySummary?.gaps || []).map((gap) => [gap.after, gap]),
        );
        const groups = [];
        let current = null;
    ...
    function renderGapCard(gap) {
        const height = Math.min(
            180,
            Math.max(54, Math.round(gap.duration_seconds / 45)),
        );
        return `
    ...
    function renderFrameScrubber() {
        ...
        const bars = counts
            ...
            .join("");
        const gaps = (framesState.daySummary?.gaps || [])
            .map((gap) => {
    ...
    function toggleFrameWarp() {
        ...
        root.querySelector("#frames-scrubber")?.classList.toggle(
            "warping",
            framesState.warping,
        );
        updateFrameToolbar();
    ...
    function scrubFramesToEvent(event) {
        ...
        const group = [...root.querySelectorAll(".frames-group-head")].find(
            (head) => head.dataset.groupKey === String(hour),
        );
    ...
    function selectFrameFromEvent(event, cursor) {
        const index = framesState.frames.findIndex(
            (frame) => frame.cursor === cursor,
        );
        if (index < 0) return;
        if (event.shiftKey && framesState.lastSelectedCursor) {
            const start = framesState.frames.findIndex(
                (frame) => frame.cursor === framesState.lastSelectedCursor,
            );
            if (start < 0) {
                framesState.selected.clear();
            ...
            for (const frame of framesState.frames.slice(a, b + 1))
                framesState.selected.add(frame.cursor);
        } else if (event.metaKey || event.ctrlKey) {
            if (framesState.selected.has(cursor))
                framesState.selected.delete(cursor);
    ...
    function updateFrameSelectionUI() {
        root.querySelectorAll(".frame-browser-tile").forEach((tile) => {
            tile.classList.toggle(
                "selected",
                framesState.selected.has(tile.dataset.frameCursor),
            );
        });
        ...
        const selectedFrames = framesState.frames.filter((frame) =>
            framesState.selected.has(frame.cursor),
        );
    ...
    function startFrameMarquee(event) {
        if (
            ...
            event.target.closest(
                ".frame-browser-tile, button, a, input, .frames-group-head, .frames-gap-card",
            )
        )
            return;
    ...
    async function deleteSelectedFrames() {
        const frames = framesState.frames.filter((frame) =>
            framesState.selected.has(frame.cursor),
        );
        if (!frames.length) return;
        if (
            !confirm(
                `Delete ${frames.length} selected frame${frames.length === 1 ? "" : "s"}? This cannot be undone.`,
            )
        )
            return;
        try {
            for (const frame of frames) {
                await api.fetchJson(
                    `/api/cameras/${encId}/frames/${encodeURIComponent(frame.day)}/${encodeURIComponent(frame.filename)}`,
                    { method: "DELETE" },
                );
            }
    ...
    function paintFrameLightbox(index) {
        ...
        const nearby = framesState.frames.slice(
            Math.max(0, index - 8),
            index + 9,
        );
        box.innerHTML = `
      ...
      <div class="frames-lb-meta">
        ...
        box.querySelector(".frames-lb-close")?.addEventListener(
            "click",
            closeFrameLightbox,
        );
        box.querySelector(".frames-lb-arrow.prev")?.addEventListener(
            "click",
            () => navigateFrameLightbox(-1),
        );
        box.querySelector(".frames-lb-arrow.next")?.addEventListener(
            "click",
            () => navigateFrameLightbox(1),
        );
        box.querySelectorAll("[data-lb-cursor]").forEach((button) =>
            button.addEventListener("click", () =>
                openFrameLightbox(button.dataset.lbCursor),
            ),
        );
    }
    ...
    function navigateFrameLightbox(delta) {
        ...
        const next = Math.max(
            0,
            Math.min(framesState.frames.length - 1, index + delta),
        );
    ...
    function scrubFrameKeyboard(direction, big) {
        ...
        const next = Math.max(
            0,
            Math.min(heads.length - 1, current + direction),
        );
    ...
    function frameGroupLabel(frame) {
        if (framesState.groupBy === "day")
            return formatDayTitle(framesState.activeDay);
        if (framesState.groupBy === "week")
            return `Week of ${formatDayShort(framesState.activeDay)}`;
    ...
    function formatDayRange(summary) {
        if (!summary?.first_captured_at || !summary?.last_captured_at)
            return "—";
    ...
    function wireSchedule() {
        ...
        document
            .querySelector("[data-revert-schedule]")
    ...
    function paintRendersList(videos) {
        ...
        list.querySelectorAll("[data-delete]").forEach((btn) => {
            btn.addEventListener("click", async () => {
                ...
                try {
                    await api.fetchJson(
                        `/api/cameras/${encId}/videos/${encodeURIComponent(filename)}`,
                        { method: "DELETE" },
                    );
                    await wireRenders();
                } catch (e) {
    ...
    function reinitStatusHtml() {
        if (reinitPhase === "saving")
            return `<span style="color:var(--soft)">Saving…</span>`;
        if (reinitPhase === "waiting")
            return `<span style="color:var(--soft)">Waiting for agent…</span>`;
        if (reinitPhase === "done") {
        ...
        if (reinitErrorMsg)
            return `<span style="color:var(--red)">${escapeHtml(reinitErrorMsg)}</span>`;
        return "";
    }
    ...
    function renderDslrSection(cfg, status) {
        ...
        const sel = (gphotoKey, field) =>
            currentValues[gphotoKey] || dslrCfg[field] || "";
        const initOpts = (gphotoKey, field, def) =>
            DSLR_INIT_CHOICES[gphotoKey]
                .map(
                    (v) =>
        ...
        const tiles = [
            statTile(
                "Battery",
                dslrSt.battery_level ? escapeHtml(dslrSt.battery_level) : null,
            ),
            statTile(
                "Available shots",
                dslrSt.available_shots != null
                    ? Number(dslrSt.available_shots).toLocaleString()
                    : null,
            ),
            statTile(
                "Shutter count",
                dslrSt.shutter_counter != null
                    ? Number(dslrSt.shutter_counter).toLocaleString()
                    : null,
            ),
        ]
            .filter(Boolean)
        ...
        const lastInit = dslrSt.last_init_at
            ? relativeTime(dslrSt.last_init_at)
            : "—";
    ...
    function renderSettingsTab() {
        const cfg = camera.config || {};
        const isGphoto2 =
            cfg.camera_backend === "gphoto2" ||
            (camera.status && camera.status.active_backend === "gphoto2");
    ...
    function wireSettings() {
        const isGphoto2 =
            (camera.config && camera.config.camera_backend === "gphoto2") ||
            (camera.status && camera.status.active_backend === "gphoto2");
        const optNum = (id) => {
        ...
        function buildDslrPayload(withReinit) {
            ...
            return {
                capture_target:
                    ...
                    "Memory card",
                drive_mode:
                    selVal("d-drivemode") || existing.drive_mode || "Single",
                focus_mode:
                    selVal("d-focusmode") || existing.focus_mode || "Manual",
        ...
        document
            ...
            .addEventListener("click", () => {
                if (
                    !confirm(
                        `Delete camera "${cameraId}"? This cannot be undone.`,
                    )
                )
                    return;
