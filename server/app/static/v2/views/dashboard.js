...
import {
  ...
  relativeTime, statusKind, renderTopbar
...
function thumbScene(camera) {
  ...
  return ["scene-day", "scene-overcast", "scene-dusk", "scene-night"][sum % 4];
}
    ...
    <div>
      ...
      <div class="cam-canvas">
        ...
        ${camera.status?.last_capture_at
          ? `<div class="cam-overlay-br"><span class="stamp">${escapeHtml(camera.status.last_capture_at.replace("T", " · ").slice(0, 19))}</span></div>`
          : ""}
      </div>
    </div>`;
}
    ...
    <a href="#/cameras/${id}" class="card" style="padding:10px;display:flex;gap:10px;align-items:center;text-decoration:none;color:var(--ink)">
      ...
      <div class="grow" style="min-width:0">
        ...
        <div class="mono" style="font-size:10px;color:var(--soft);margin-top:3px">
          ${statusKind(camera.status) === "live" ? '<span style="color:var(--green)">● LIVE</span>'
            : statusKind(camera.status) === "failed" ? '<span style="color:var(--red)">✕ ERROR</span>'
            : '<span>○ ' + (statusKind(camera.status) === "paused" ? "PAUSED" : "OFFLINE") + '</span>'}
...
async function renderDashboard(root) {
  ...
  async function load() {
    try {
      ...
      const totalImages = cameras.reduce((s, c) => s + (c.image_count || 0), 0);
      const totalQueued = cameras.reduce((s, c) => s + (c.status?.pending_count || 0), 0);

      const storageUsed = stats.storage_bytes;
      const storageCap = stats.storage_capacity_bytes;
      const storagePct = (storageUsed && storageCap) ? Math.round((storageUsed / storageCap) * 100) : null;
        ...
        <div style="padding:20px 24px;border-bottom:1px solid var(--border)">
          ...
          <div class="stat-grid" style="margin-top:12px">
            ...
            ${statCard("Storage",
                ...
                storageCap ? `${storagePct}% of ${formatBytes(storageCap)}` : "",
                storagePct != null && storagePct > 85 ? "bad" : storagePct != null && storagePct > 70 ? "warn" : null)}
            ${statCard("Errors",
                String(failingCount),
