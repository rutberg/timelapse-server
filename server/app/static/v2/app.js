...
export function statusKind(status) {
    ...
    if (status.in_schedule === false) return "paused";
    return "live";
}
...
export function renderTopbar(crumbs, rightHtml = "") {
    ...
    const items = (crumbs || [])
        .map((c, i) => {
            ...
            if (href && !isLast)
                return `${sep}<a href="${escapeHtml(href)}">${label}</a>`;
...
export function openModal(html, opts = {}) {
    ...
    backdrop.addEventListener("click", (e) => {
        if (e.target.dataset.close || e.target.closest("[data-modal-close]"))
            close();
    });
...
async function render() {
    ...
    const isActive = () => myToken === navToken;
    ...
    for (const [prefix, view] of views.entries()) {
        if (hash.startsWith(prefix)) {
            ...
            try {
                const cleanup = await view(root, hash, isActive);
                // If a newer render started while we awaited, this render is stale:
                // discard our cleanup (call it now to release any timers/listeners)
                // instead of letting it overwrite the active view's cleanup.
                if (!isActive()) {
